...
export default function DoctorDetailPage() {
  ...
  if (!isNew && isLoading) return <div className="text-center py-20 text-gray-400">Loading...</div>
  ...
  return (
    <div>
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
        ...
        <span>/</span>
        <span className="text-gray-700">{isNew ? 'New Doctor' : `Dr. ${currentDoctor.first_name || ''} ${currentDoctor.last_name || ''}`}</span>
      </div>

      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold text-gray-900">
          {isNew ? 'Add New Doctor' : `Dr. ${currentDoctor.first_name || ''} ${currentDoctor.last_name || ''}`}
        </h1>
        {!isNew && (
          <div className="flex gap-2">
            <Link to={`/doctors/${id}/documents`} className="btn-secondary">Documents</Link>
            <Link to={`/doctors/${id}/workflow`} className="btn-secondary">Workflow</Link>
            <Link to={`/doctors/${id}/tdi`} className="btn-secondary">TDI</Link>
          </div>
        )}
      </div>
      ...
      {!isNew && doctor && !doctor.tdi_completed && (
        <div className="mb-4 p-3 bg-orange-50 border border-orange-200 rounded-md flex items-center justify-between">
          <div className="text-sm text-orange-800">
            <strong>⚠️ TDI Application Pending:</strong> Texas Standardized Credentialing Application has not been signed yet. Only the doctor can sign this form.
          </div>
          <Link to={`/doctors/${id}/tdi`} className="btn-sm btn-secondary ml-4 shrink-0">Manage TDI →</Link>
        </div>
      )}
      ...
      <div className="card mb-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input className="input" value={form.npi || ''} onChange={e => setForm(f => ({ ...f, npi: e.target.value }))} />
          </div>
          <div>
            <label className="label">CAQH ID</label>
            <input className="input" value={form.caqh_id || ''} onChange={e => setForm(f => ({ ...f, caqh_id: e.target.value }))} />
          </div>
          <div>
            <label className="label">Primary Specialty</label>
            <input className="input" value={form.primary_specialty || ''} onChange={e => setForm(f => ({ ...f, primary_specialty: e.target.value }))} />
          </div>
          <div>
            <label className="label">Credentialing Status</label>
            <select className="input" value={form.credentialing_status || 'pending'} onChange={e => setForm(f => ({ ...f, credentialing_status: e.target.value }))}>
              {CREDENTIALING_STATUSES.map(s => <option key={s} value={s}>{STATUS_LABELS[s]}</option>)}
            </select>
          </div>
          {user?.role === 'admin' && (
            <div>
              <label className="label">Assigned Worker</label>
              <select className="input" value={form.assigned_worker_id || ''} onChange={e => setForm(f => ({ ...f, assigned_worker_id: e.target.value || null }))}>
          ...
          <div>
            ...
            <input type="date" className="input" value={form.recredentialing_due_date || ''} onChange={e => setForm(f => ({ ...f, recredentialing_due_date: e.target.value }))} />
          </div>
        </div>
        <div className="mt-4 flex items-center gap-3">
          ...
          {msg && <span className={`text-sm ${msg.includes('fail') || msg.includes('Error') ? 'text-red-600' : 'text-green-600'}`}>{msg}</span>}
        </div>
      </div>
      ...
      {!isNew && (
        <>
          ...
          <div>
            {activeTab === 'personal' && <PersonalInfoTab doctorId={id} doctor={doctor} onUpdate={f => { setForm(p => ({ ...p, ...f })); doctors.update(id, f).then(() => qc.invalidateQueries(['doctor', id])) }} />}
            ...
            {activeTab === 'locations' && <PracticeLocationsTab doctorId={id} />}
            {activeTab === 'hospitals' && <HospitalAffiliationsTab doctorId={id} />}
            {activeTab === 'contacts' && <CredentialingContactsTab doctorId={id} />}
            {activeTab === 'insurance' && <LiabilityInsuranceTab doctorId={id} />}
            {activeTab === 'employment' && <EmploymentHistoryTab doctorId={id} />}
