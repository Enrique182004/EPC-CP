...
export default function EducationTab({ doctorId }) {
  return (
    <SubResourceTab
      ...
      fields={[
        { name: 'education_type', label: 'Type', required: true, type: 'select', options: EDUCATION_TYPES },
      ...
      renderRow={(row) => (
        <div>
          <span className="font-medium capitalize">{row.education_type.replace('_', ' ')}</span>
          <span className="mx-1 text-gray-400">—</span>
          <span>{row.institution_name}</span>
          {row.degree && <span className="text-gray-500 ml-2">({row.degree})</span>}
          {row.end_date && <span className="text-gray-400 ml-2 text-xs">{row.end_date?.slice(0,4)}</span>}
        </div>
      )}
    />
  )
