...
export default function DisclosureTab({ doctorId }) {
  ...
  return (
    <div className="card">
      ...
      <p className="text-sm text-gray-500 mb-4">Answer each question. If "Yes", please provide an explanation.</p>
      <div className="space-y-4">
        {DISCLOSURE_QUESTIONS.map(q => {
          const current = currentAnswers(q.key)
          return (
            <div key={q.key} className="border border-gray-100 rounded-lg p-4 hover:bg-gray-50">
              <p className="text-sm font-medium text-gray-800 mb-2">{q.label}</p>
              <div className="flex gap-6 mb-2">
                <label className="flex items-center gap-2 text-sm">
                  <input type="radio" name={q.key} value="no" checked={!current.answer} onChange={() => setAnswer(q.key, 'answer', 0)} />
                  No
                </label>
                <label className="flex items-center gap-2 text-sm">
                  <input type="radio" name={q.key} value="yes" checked={!!current.answer} onChange={() => setAnswer(q.key, 'answer', 1)} />
                  Yes
                </label>
              </div>
              {!!current.answer && (
                <textarea
                  ...
                  onChange={e => setAnswer(q.key, 'explanation', e.target.value)}
                />
              )}
            </div>
          )
      ...
      <div className="mt-4 flex items-center gap-3">
        <button className="btn-primary" onClick={handleSave}>Save Disclosures</button>
