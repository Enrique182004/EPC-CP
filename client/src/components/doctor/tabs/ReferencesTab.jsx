...
export default function ReferencesTab({ doctorId }) {
  return (
    <SubResourceTab
      ...
      updateFn={(subId, data) =>
        professionalReferences.update(doctorId, subId, data)
      }
      ...
      renderRow={(row) => (
        <div>
          ...
          {row.specialty && (
            <span className="text-gray-500 ml-2">— {row.specialty}</span>
          )}
          {row.relationship && (
          ...
          {row.email && (
            <span className="text-gray-400 ml-2 text-xs">{row.email}</span>
          )}
        </div>
      )}
    />
