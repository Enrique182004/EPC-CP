...
export default function HospitalAffiliationsTab({ doctorId }) {
  return (
    <SubResourceTab
      ...
      updateFn={(subId, data) =>
        hospitalAffiliations.update(doctorId, subId, data)
      }
      ...
      fields={[
        ...
        {
          name: "privileges_requested",
          label: "Privileges Requested",
          type: "textarea",
          span: "full",
        },
      ]}
      renderRow={(row) => (
        <div>
          ...
          {row.department && (
            <span className="text-gray-500 ml-2">— {row.department}</span>
          )}
          {row.city && (
