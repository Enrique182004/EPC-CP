...
router.post('/register', authenticate, requireRole('admin'), async (req, res, next) => {
  try {
    ...
    const result = User.create({ email, password_hash, role: role || 'worker', name, phone });
    const newUser = User.findById(result.lastInsertRowid);
    res.status(201).json({ user: { id: newUser.id, email: newUser.email, role: newUser.role, name: newUser.name } });
  } catch (err) { next(err); }
});
