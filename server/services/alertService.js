...
async function sendAlert({ to, subject, htmlBody, doctor_id, alert_type, subject_id }) {
  try {
    await sendMail({ to, subject, html: htmlBody });
    AlertLog.log({ doctor_id, alert_type, subject_id, recipient_email: to, success: true });
  } catch (err) {
    AlertLog.log({ doctor_id, alert_type, subject_id, recipient_email: to, success: false, error_message: err.message });
  }
}
  ...
  for (const { days, label } of THRESHOLDS) {
    ...
    for (const row of insurances) {
      ...
      if (!AlertLog.wasAlertSent(row.id, alertType, row.ins_id)) {
        ...
        const html = emailHtml(`Malpractice Insurance Expiration Warning (${label.replace('_', ' ')})`, body);

        const recipients = [row.personal_email, row.work_email, row.worker_email].filter(Boolean);
        for (const email of [...new Set(recipients)]) {
          await sendAlert({ to: email, subject, htmlBody: html, doctor_id: row.id, alert_type: alertType, subject_id: row.ins_id });
        }
      }
    }
    ...
    for (const row of licenses) {
      ...
      if (!AlertLog.wasAlertSent(row.id, alertType, row.pid_id)) {
        ...
        const html = emailHtml(`${row.id_type} Expiration Warning (${label.replace('_', ' ')})`, body);

        const recipients = [row.personal_email, row.work_email, row.worker_email].filter(Boolean);
        for (const email of [...new Set(recipients)]) {
          await sendAlert({ to: email, subject, htmlBody: html, doctor_id: row.id, alert_type: alertType, subject_id: row.pid_id });
        }
      }
    }
    ...
    for (const row of recreds) {
      ...
      if (!AlertLog.wasAlertSent(row.id, alertType, null)) {
        ...
        const html = emailHtml(`Re-credentialing Reminder (${label.replace('_', ' ')})`, body);

        const recipients = [row.personal_email, row.work_email, row.worker_email].filter(Boolean);
        for (const email of [...new Set(recipients)]) {
          await sendAlert({ to: email, subject, htmlBody: html, doctor_id: row.id, alert_type: alertType, subject_id: null });
        }
      }
    }
  }
