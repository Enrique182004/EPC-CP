...
const findAll = ({ search, status, workerId } = {}) => {
  ...
  const params = [];
  if (search) { q += ` AND (d.first_name LIKE ? OR d.last_name LIKE ? OR d.npi LIKE ? OR d.caqh_id LIKE ?)`; const s = `%${search}%`; params.push(s,s,s,s); }
  if (status) { q += ` AND d.credentialing_status = ?`; params.push(status); }
  if (workerId) { q += ` AND d.assigned_worker_id = ?`; params.push(workerId); }
  q += ` ORDER BY d.last_name, d.first_name`;
  return db.prepare(q).all(...params);
};

const findById = (id) =>
  db.prepare(`SELECT d.*, u.name as worker_name, u.email as worker_email FROM doctors d LEFT JOIN users u ON d.assigned_worker_id = u.id WHERE d.id = ?`).get(id);

const create = (fields) => {
  const cols = Object.keys(fields);
  const vals = Object.values(fields);
  const stmt = db.prepare(`INSERT INTO doctors (${cols.join(', ')}) VALUES (${cols.map(() => '?').join(', ')})`);
  return stmt.run(...vals);
};

const update = (id, fields) => {
  const allowed = [
    'first_name','middle_name','last_name','suffix','date_of_birth','ssn_last4','gender',
    'home_address','home_city','home_state','home_zip','home_phone','cell_phone','personal_email',
    'npi','caqh_id','work_email','primary_specialty','credentialing_status','assigned_worker_id',
  ...
  db.prepare(`UPDATE doctors SET ${sets.join(', ')} WHERE id = ?`).run(...vals, id);
};
...
module.exports = { findAll, findById, create, update, remove, findExpiringLicenses, findExpiringInsurance, findExpiringRecredentialing };
